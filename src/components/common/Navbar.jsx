...
function Navbar() {
  ...
  const [subLinks, setSubLinks] = useState([]); // Ensures it's always an array
    ...
    (async () => {
      ...
      try {
        ...
        if (res?.data?.data && Array.isArray(res.data.data)) {
          ...
          setSubLinks([]); // Set empty array if response is invalid
        }
      } catch (error) {
        console.error("Could not fetch Categories.", error);
        setSubLinks([]); // Handle API failure gracefully
      }
      setLoading(false);
    })();
  }, []);
  ...
  return (
    ...
    >
      ...
      {showDropDown && (
        <div className="w-full md:hidden p-4 h-fit bg-richblack-800 z-50 absolute top-[55px] text-white border border-white">
